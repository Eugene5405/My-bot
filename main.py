...
def start(m):
    ...
    if uid not in U:
        bot.send_message(m.chat.id,"🌐 Выбери язык / Choose language / Изабери језик:",reply_markup=lang_kb())
        return
    bot.send_message(m.chat.id,LANGS.get(U.get(uid,{}).get("lang","ru"),LANGS["ru"])["welcome"],reply_markup=main_kb(uid))
...
def loc_h(m):
    ...
    U[uid]=cur
    save()
    bot.send_message(m.chat.id,f"✅ Сохранено! {tz} 📍",reply_markup=main_kb(uid))
...
def profi(m):
    ...
    if cmd=="debug":
        bot.send_message(m.chat.id,f"DEBUG lang={lang} data={U.get(s_uid)}")
        return
    if cmd=="location":
