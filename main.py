...
import threading, telebot, requests, json, os
...
def cmd_start(m): bot.send_message(m.chat.id,"Hello! Welcome to Time & Weather Bot.",reply_markup=main_kb())
...
def cmd_help(m): bot.send_message(m.chat.id,"/watch - time\n/weather - weather\n/weekweather\n/hourly\n/tomorrow\n/rain\n/air\n/uv\n/sun\n/wind\n/alerts",reply_markup=main_kb())
@bot.message_handler(commands=['weather'])
def cmd_weather(m):
 loc=get_loc(m.from_user.id); w=get_weather(loc['lat'],loc['lon']); c=w['current']
 bot.send_message(m.chat.id,f"Weather: {c['temperature_2m']}C, {c['relative_humidity_2m']}%, Wind {c['wind_speed_10m']} km/h",reply_markup=weather_kb())
@bot.message_handler(content_types=['location'])
def loc_handler(m):
 d=get_weather(m.location.latitude,m.location.longitude)
 user_locations[str(m.from_user.id)]={"lat":m.location.latitude,"lon":m.location.longitude,"
